...
FOLDER_PATH_RESULTS = os.path.join(MAIN_FILE, "Last-Minute/Long_Prediction")
FOLDER_PATH_DATABASE = os.path.join(MAIN_FILE, "DB-EB")
...
def chartjs_to_endpoint(data, customer_id, error_msg=None, use_company_api=True):
    ...
    try:
        if use_company_api:
            ...
            # into DataFrame
            save_file_path = os.path.join(FOLDER_PATH_DATABASE, f"database_early_bird_{customer_id}.json")
    ...
    if len(content_data) > 0:
        ...
        for i in range(len(adaptive_suggestion_df)):
            ...
            if adaptive_suggestion_df.iloc[i, 0] != "" and adaptive_suggestion_df.iloc[i, 0] > datetime.now() + timedelta(days=30):
                sub_process_dict_copy["forecast_date"] = adaptive_suggestion_df.iloc[i, 0].strftime("%Y-%m-%d").strftime("%Y-%m-%d %H:%M:%S")
            else:
                continue
