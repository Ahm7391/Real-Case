...
CUSTOMERS_LIST : str | None = os.getenv("TOTAL_ROOM_NUMBER_URL")
API_KEY = os.getenv("TOKEN_SERVER")

CURR_FILE = os.path.abspath(__file__)
CURR_DIR = os.path.dirname(CURR_FILE)
MAIN_FILE = Path(__file__).resolve().parents[1]
FOLDER_PATH_CP = os.path.join(MAIN_FILE, "Last-Minute/Checkpoint")
...
def customers_data_updater():
    ...
    if not CUSTOMERS_LIST:
        raise RuntimeError("TOTAL_ROOM_NUMBER_URL environment variable is not set")
    try:
...
def checkpoint_checker():
    if not os.path.exists("customers_main_data.json"):
        ...
        if int(delta_time_check.days) >= 30:
            ...
            latest_checkpoint = os.path.join(MAIN_FILE, "checkpoint_save.txt")
