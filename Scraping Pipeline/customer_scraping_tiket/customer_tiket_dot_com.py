...
def scrape_hotel_price(
    ...
    while date_avail == False:
        ...
        except (WebDriverException, Exception) as get_err:
            log.error(f"Error navigating to {hotel_url_new}: {get_err}")
            return None, {}
        ...
        if only_once_pname == True: # FIND PROPERTY NAME
            ...
            try:
                ...
                log.warning("Throwing error TimeoutException, NoSuchElementException")
                property_name = property_id
    ...
    while date_avail == False:
        ...
        if only_once_pname == True: # FIND PROPERTY NAME
            ...
            try:
                ...
                # if not property_name:
                #     log.warning("Could not find property name — page may not have loaded properly")
                log.warning("Throw error TimeoutException, NoSuchElementException")
                property_name = property_id
            only_once_pname = False
...
def save_load_database(focus_df):
    ...
    if os.path.exists("scraping_customer_tiket_lm.pkl"):
        loaded_df = pd.read_pickle("scraping_customer_tiket_lm.pkl")
        data_df2 = pd.concat([data_df2, loaded_df])
        data_df2 = data_df2.reset_index(drop=True)
        data_df2.to_pickle("scraping_customer_tiket_lm.pkl")
...
def save_load_database_eb(focus_df):
    # data_df2 = pd.DataFrame()
    data_df2 = focus_df.copy()
    if os.path.exists("scraping_customer_tiket_eb.pkl"):
        loaded_df = pd.read_pickle("scraping_customer_tiket_eb.pkl")
        data_df2 = pd.concat([data_df2, loaded_df])
        data_df2 = data_df2.reset_index(drop=True)
        data_df2.to_pickle("scraping_customer_tiket_eb.pkl")
        ...
        try:
            ...
            if os.path.exists("checkpoint_save_customer_url.txt"):
                ...
                for idx, item in enumerate(url_list):
                    if get_base_url(item["url"]) == get_base_url(cross_check):
                        checkpoint_idx = idx
                        break
            ...
            targets_dict = url_list[start_index : start_index + MAX_PROPERTIES]
            targets = [item["url"] for item in targets_dict]
            targets_id = [item["customer_id"] for item in targets_dict]
            ...
            for i, url in enumerate(targets, 1):
                try:
                    # Verify driver connection health before starting property
                    try:
                    ...
                    for vii in period_cycling:
                        ...
                        if vii <= 7:
                            record, pending_lm = scrape_hotel_price(driver, url, vii, targets_id[i-1], trial_seed=2)
                            if record:
                                new_records.append(record)
                            ...
                            record_eb, pending_eb = scrape_hotel_price_early_book(driver, url, vii, targets_id[i-1], trial_seed=2)
                            if record_eb:
                                new_records_eb.append(record_eb)
                            if pending_eb:
                                pending_list_eb.append(pending_eb)
...
if __name__ == "__main__":
    ...
    try:
        ...
        driver, pending_list_eb_extend, pending_list_lm_extend = competitor_only(driver, url_find, manual_find)
