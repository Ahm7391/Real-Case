...
def clean_text(text: str) -> set:
    ...
    stopwords = {'by', 'and', 'amp', 'the', 'hotel', 'id', 'com', 'tiket', 'www', 'html', 'indonesia', 'en'}
    return {w.rstrip('s') for w in words if w not in stopwords}

def extract_slug_words(url: str) -> set:
    """Extract clean words from Tiket.com URL slug."""
    path_last = url.rstrip('/').split('?')[0].split('/')[-1]
    slug = re.sub(r'-\d+$', '', path_last)
    slug = re.sub(r'[^a-zA-Z0-9]', ' ', slug.lower())
    words = set(slug.split())
    stopwords = {'by', 'and', 'amp', 'the', 'hotel', 'id', 'com', 'tiket', 'www', 'html', 'indonesia', 'en', 'oyo', 'capital', 'o'}
    return {w.rstrip('s') for w in words if w not in stopwords}
...
def match_urls(manual_search_path: str, seen_url_path: str, output_path: str):
    ...
    with open(seen_url_path, 'r', encoding='utf-8') as f:
        ...
        if isinstance(seen_urls_data, dict):
            urls = list(seen_urls_data.keys())
        else:
            urls = list(seen_urls_data)
    ...
    print(f"Assigned {matched_count}/{n_urls} URLs to {output_path}.")
