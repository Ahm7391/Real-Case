...
WEBHOOK_URL = os.getenv("INGEST_URL")
...
DATABASE_TIKET_EB = os.path.join(FOLDER_PATH_TIKET, "scraping_database_tiket_eb.pkl")
DATABASE_TIKET_LM = os.path.join(FOLDER_PATH_TIKET, "scraping_database_tiket_lm.pkl")
BATCH_ENTRY = 100
...
main_dat = {
    "database_booking_dot_com": ["scraping_database_lm.pkl", "scraping_database_eb.pkl"],
...
def load_allowed_property_names(filename="property_with_competitor.json"):
        ...
        if see_name not in seen_names:
            ...
            if len(res_df) > 0:
                for ii in range(len(res_df)):
                    ...
                    batch_scrap2["property_name"] = res_df.loc[ii, "property_name"]
