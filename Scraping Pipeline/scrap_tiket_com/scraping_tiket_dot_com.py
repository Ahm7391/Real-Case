...
def recycle_driver(driver, download_folder: str, headless: bool):
    """Quit the current Chrome and start a fresh one.

    tiket.com is a heavy SPA; one long-lived Chrome steadily accumulates memory
    over many page loads. Recycling every DRIVER_RECYCLE_EVERY properties keeps
    RAM bounded on the VPS. Returns the new driver — caller must reassign it.
    """
    if driver:
        try:
            driver.quit()
    ...
    return build_driver(download_folder, headless=headless)
...
def build_search_url(incoming_url, checkin: str, checkout: str) -> str:
    ...
    Any pre-existing query string on incoming_url is discarded; the path
    (including the trailing numeric property ID) is preserved.
    """
    base = urlunparse(urlparse(incoming_url)._replace(query="", fragment="")).rstrip("/")
    params = (
...
def scrape_agoda(target_url, tiket_address, period_days, mode_main):
    """
    ...
    # ================================== CORE FUNCTIONS ================================================
    def scrape_agoda_info(wait_specs, driver, url_tgt, address, deltatime, mode):
        focus_url = url_tgt
        ...
        for ii in range(3):
            ...
            if ii == 0:
                ...
                todays_date = todays_date + timedelta(days=deltatime)
        ...
        if check_result == False or mismatch_address == True:
            ...
            for card in room_cards:
                # Fresh dict per card — otherwise every appended row shares one
                # reference and all rooms end up showing the last room's data.
                room_data = {
                    "room_name":           None,
                    "children_policy":     None,
                    "breakfast":           None,
    ...
    try:
        ...
        if content_exists == True:
            ...
            if best_score >= MATCH_THRESHOLD:
                ...
                scrape_res = scrape_agoda_info(agoda_wait, agoda_driver, best_match["url"], 
                                               tiket_address, deltatime=period_days, mode=mode_main)
            ...
            #     for i in link_list:
            #         print(f"Exploring {i}")
            #         scrape_res = scrape_agoda_info(agoda_wait, agoda_driver, i, 
            #                                        tiket_address, deltatime=period_days, mode=mode_main)
            #         print(f"scrape_res is: \n{scrape_res}")
            # else:
            ...
            #     return 0
             ...
             # ── Pick the single best match by score ──────────────────────────
            # focus_list is already sorted by score DESC. If the top score is
            # tied between 2+ properties we can't safely tell which is the real
            # one, so we bail out (return 0) rather than risk scraping the wrong
            # property and polluting the database with sporadic matches.
            if not focus_list:
                print("Cannot find that property on Agoda!")
                agoda_driver.quit()
                return 0

            if len(focus_list) >= 2 and focus_list[0]["score"] == focus_list[1]["score"]:
                print(
                    f"[✗] Ambiguous match — highest score {focus_list[0]['score']}% is tied "
                    f"across multiple properties. Cannot confirm the correct one, returning 0."
                )
                agoda_driver.quit()
                return 0

            best = focus_list[0]
            print(f"[OK] Best match: '{best['name']}' ({best['score']}%) → {best['link']}")
            scrape_res = scrape_agoda_info(agoda_wait, agoda_driver, best["link"],
                                           tiket_address, deltatime=period_days, mode=mode_main)
            print(f"scrape_res is: \n{scrape_res}")
...
def scrape_hotel_price(
    ...
    ckin_dt, ckout_dt = get_checkin_checkout(offset_days=period_days)
    ...
    while date_avail == False:
        ...
        if only_once_pname == True: # FIND PROPERTY NAME
            ...
            time.sleep(1)
            # ANNOYING POPUPS DETECTION (dynamic -> elements name could be different due to updated promotions)
            try: 
                annoying_popups = WebDriverWait(driver, 5).until(
                    EC.element_to_be_clickable(
                        (By.CSS_SELECTOR, "div[class*='styles-module__container'] button[class*='close_button']")
                    )
                )
                try:
                    annoying_popups.click()
                except Exception:
                    driver.execute_script("arguments[0].click();", annoying_popups)
                log.info("Annoying promotional popup detected and closed.")
                time.sleep(1)
            except TimeoutException:
                log.info("Luckily no annoying popups detected.")
        ...
        try:
            ...
            pending_dict = ({'property_name': property_name, 'address':tiket_address, 'period_days':period_days}) # APPEND FOR LATER SEARCH WITH AGODA
            results = []
            data = {"room_type": None, "price": None, "notes": None, "date_scrapped": dt_now_rev, 'target_date': new_ckin, 'category':'last-minute'}
            results.append(data)
            ...
            try:
                ...
                try:
                    price_el = row.find_element(By.CSS_SELECTOR,
                        "div.RatePlanPrice_final_price_wrapper__t1r9i div[class*='Text_variant_price__']")
...
def scrape_hotel_price_early_book(
    ...
    hotel_url_new = build_search_url(hotel_url, checkin=ckin_dt, checkout=ckout_dt)
    # dt = datetime.strptime(ckin_dt, "%Y-%m-%d")  # use your actual format
    # day_weekday = dt.strftime("%A")
...
def cleaning_data(data_df):
    ...
    # print(f"after cleaning dataframe the content is : \n{data_df2}")
    return data_df2
...
def manual_scrape(driver, target_property):
    INTENDED_PROPERTY = target_property
    # Devs can tweak this list later: any of these appearing in the
    # item-location string counts as a location match.
    LOCATION_KEYWORDS = ["indonesia"]
    MATCH_THRESHOLD = 0.85   # 90% name similarity

    # ============================= HELPER FUNCTION SECTION =============================
    def name_match_score(query: str, candidate: str) -> float:
        """Similarity score between the typed query and a suggested property name (0..1)."""
        return SequenceMatcher(None, query.lower().strip(), candidate.lower().strip()).ratio()

    def normalize_tokens(text: str) -> list[str]:
        """Lowercase word tokens, punctuation stripped (apostrophes/hyphens split out)."""
        return re.findall(r"\w+", text.lower())

    def consecutive_word_match(query: str, candidate: str, tok_threshold: float = 0.8,
                               exact: bool = False) -> bool:
        """True if the typed words appear as a CONSECUTIVE run inside the candidate name.

        e.g. 'D'jineng Rice' → matches 'D'jineng Rice Terrace Canggu' (contiguous, in order).
        exact=True  → tokens must match verbatim (strong signal).
        exact=False → per-token fuzzy (>= tok_threshold) so diacritics/apostrophes/typos
                      don't break an otherwise-consecutive match.
        """
        q    = normalize_tokens(query)
        cand = normalize_tokens(candidate)
        if not q or len(q) > len(cand):
            return False
        for start in range(len(cand) - len(q) + 1):
            window = cand[start:start + len(q)]
            if exact:
                if all(a == b for a, b in zip(q, window)):
                    return True
            else:
                if all(SequenceMatcher(None, a, b).ratio() >= tok_threshold for a, b in zip(q, window)):
                    return True
        return False

    def tiered_consecutive_pick(cand_data, query):
        """Exact-consecutive beats fuzzy-consecutive.

        cand_data: list of (item, name, loc_ok). Returns (status, item, name) where
        status is 'unique' | 'ambiguous' | 'none'. We first try EXACT contiguous token
        matches; only if there are zero exact hits do we fall back to fuzzy. This stops a
        real property (e.g. 'Manca Villa') from being lost to look-alikes ('Maca Villas',
        'Kanca Villas') that only clear the fuzzy bar.
        """
        for use_exact in (True, False):
            hits = [(it, nm) for (it, nm, ok) in cand_data
                    if ok and consecutive_word_match(query, nm, exact=use_exact)]
            if len(hits) == 1:
                return ("unique", hits[0][0], hits[0][1])
            if len(hits) > 1:
                return ("ambiguous", None, None)
        return ("none", None, None)

    def location_matches(location_text: str, keywords: list[str]) -> bool:
        """True if the item-location string contains any of the allowed keywords."""
    ...
    try:
        ...
        # 1) Wait until the destination-input element appears AND is clickable
        destination_input = ms_wait.until(
            EC.element_to_be_clickable((By.CSS_SELECTOR, "div[data-testid='destination-input']"))
        )
        print("destination-input found!")
        ...
        search_box = ms_wait.until(
            EC.element_to_be_clickable(
                    (By.CSS_SELECTOR, "input[data-testid='destination-search-box']")
                )
        )
        print("destination-search-box found!")

        # 3) Type WORD BY WORD. Starting after the 2nd word, check the live
        #    suggestions for a CONSECUTIVE word match (+ correct location). If
        #    exactly ONE suggestion qualifies, pick it immediately and stop
        #    typing. If several qualify, keep typing to narrow it down.
        search_box.clear()
        words = INTENDED_PROPERTY.split()
        typed_so_far = ""
        best_item = None
        best_score = 0.0
        early_pick = False
        ...
        for w_idx, word in enumerate(words, start=1):
            chunk = (" " if typed_so_far else "") + word
            for ch in chunk:
                search_box.send_keys(ch)
                time.sleep(random.uniform(0.2, 0.9))
            typed_so_far = (typed_so_far + " " + word).strip()

            # Only start scoring once at least 2 words are in the box
            if w_idx < 2:
                continue

            time.sleep(2)  # let the suggestion dropdown refresh
            live_items = driver.find_elements(By.CSS_SELECTOR, "div[data-testid='hotel-list-item']")

            cand_data = []
            for item in live_items:
                try:
                    name_text = item.find_element(By.CSS_SELECTOR, "p[data-testid='item-name']").text.strip()
                except NoSuchElementException:
                    continue
                try:
                    loc_text = item.find_element(By.CSS_SELECTOR, "p[data-testid='item-location']").text.strip()
                except NoSuchElementException:
                    loc_text = ""
                cand_data.append((item, name_text, location_matches(loc_text, LOCATION_KEYWORDS)))

            status, hit_item, matched_name = tiered_consecutive_pick(cand_data, typed_so_far)
            if status == "unique":
                best_item = hit_item
                early_pick = True
                print(f"[EARLY PICK] Unique consecutive match after {w_idx} words "
                      f"({typed_so_far!r} → {matched_name!r}) — stop typing.")
                break
            elif status == "ambiguous":
                print(f"[{w_idx} words] multiple consecutive matches — ambiguous, keep typing.")
            else:
                print(f"[{w_idx} words] no consecutive match yet — keep typing.")

        # 4) No unique early pick → the full name is now typed. Look for a
        #    property whose name contains the typed words CONSECUTIVELY (the
        #    candidate may be much longer than what we typed) + correct location.
        #      • exactly 1 such match → treat it as the same property, pick it
        #      • 2+ matches (ambiguity)  → dismiss, do NOT pick
        #      • 0 matches               → last resort: full-string >= MATCH_THRESHOLD
        if not early_pick:
            print(f"typed full search string: {INTENDED_PROPERTY!r}")
            time.sleep(3)
            try:
                items = ms_wait.until(
                    EC.presence_of_all_elements_located(
                        (By.CSS_SELECTOR, "div[data-testid='hotel-list-item']")
                    )
                )
            except TimeoutException:
                items = []

            print(f"found {len(items)} hotel-list-item suggestions")

            # 4a) Primary: exact-consecutive beats fuzzy-consecutive (unique only)
            cand_data = []   # cached (item, name, loc_ok) for the 4b fallback
            for idx, item in enumerate(items, start=1):
                ...
                loc_ok = location_matches(loc_text, LOCATION_KEYWORDS)
                cand_data.append((item, name_text, loc_ok))
                print(f"  [{idx}] {name_text!r} | loc={loc_text!r} | loc_ok={loc_ok}")

            status, hit_item, matched_name = tiered_consecutive_pick(cand_data, INTENDED_PROPERTY)
            if status == "unique":
                best_item = hit_item
                print(f"[FULL-TYPED MATCH] Unique consecutive match ({INTENDED_PROPERTY!r} → {matched_name!r}).")
            elif status == "ambiguous":
                print("[DISMISS] Multiple consecutive matches after full typing — ambiguous, not picking.")
            else:
                # 4b) Last resort (0 consecutive): full-string similarity + location
                print("[FALLBACK] No consecutive match — trying full-string similarity scoring.")
                for item, name_text, loc_ok in cand_data:
                    score = name_match_score(INTENDED_PROPERTY, name_text)
                    print(f"       {name_text!r} | score={score:.2f} | loc_ok={loc_ok}")
                    if score >= MATCH_THRESHOLD and loc_ok and score > best_score:
                        best_score = score
                        best_item = item

        # 6) Click the winner (no href on these items, so click the element itself)
        if best_item is not None:
            print(f"MATCH → clicking (early_pick={early_pick}, best_score={best_score:.2f})...")
            driver.execute_script("arguments[0].scrollIntoView({block: 'center'});", best_item)
            time.sleep(1)
            # JS click is more reliable than .click() on these UI-heavy suggestion rows
            driver.execute_script("arguments[0].click();", best_item)
            print("clicked matched property.")
        else:
            raise RuntimeError(
                "[DEV NOTE] Intended property not found: no hotel-list-item passed the "
                "unique consecutive-word + location match, nor the "
                f">={MATCH_THRESHOLD:.0%} full-string fallback. Exiting gracefully."
            )

        try:
            search_button = ms_wait.until(
                EC.element_to_be_clickable(
                    (By.CSS_SELECTOR, "button[data-testid='search-button-lp']")
                )
            )
            print("search button found!")

            driver.execute_script("arguments[0].scrollIntoView({block: 'center'});", search_button)
            time.sleep(random.uniform(0.5, 1.5))   # small human-like pause before clicking

            try:
                search_button.click()
            except Exception:
                # fallback: JS click if something overlaps the button (e.g. the ripple span)
                driver.execute_script("arguments[0].click();", search_button)
            print("clicked search button — search started.")
        except TimeoutException:
            print(
                "[DEV NOTE] 'Let's Search' button (search-button-lp) never appeared "
                f"within {PAGE_TIMEOUT}s — form may not have auto-filled. Exiting gracefully."
            ) 

        # the search may open the property page in a NEW TAB — switch to it if so
        time.sleep(3)
        if len(driver.window_handles) > 1:
            print(f"{len(driver.window_handles)} tabs open, switching to the newest one...")
            driver.switch_to.window(driver.window_handles[-1])

        # wait until the property page actually loads (URL contains the PDP path)
        ms_wait.until(lambda d: "tiket.com" in d.current_url and d.current_url != "https://www.tiket.com/en-id/hotel")

        property_url = driver.current_url
        print(f"property page URL: {property_url}")
        # driver.quit()
        # print("driver closed.")
        return property_url

    except Exception as e:
        print(f"[CAUTION][MANUAL SCRAPE ERROR] due to {e}")
        return None

def all_properties_scrape(driver):
    new_urls = []
    debug_counter = 0
    ...
    cross_check = ""   # empty until a checkpoint file exists → comparisons stay False
    ...
    pending_list_lm = []
    pending_list_eb = []
    recycle_dir = os.path.dirname(os.getcwd())   # same dir main passes to build_driver
    processed = 0   # properties scraped since last driver recycle
    for i, url in enumerate(targets, 1):
        # Recycle every N properties to keep Chrome memory bounded
        # (skip on the first iteration — driver is already fresh)
        if processed > 0 and processed % DRIVER_RECYCLE_EVERY == 0:
            driver = recycle_driver(driver, recycle_dir, IS_VPS)

        log.info(f"[{i}/{len(targets)}] Scraping …")
        record, pending_lm = scrape_hotel_price(driver, url, period_days=1, trial_seed=2)
        record_eb, pending_eb = scrape_hotel_price_early_book(driver, url, period_days=30, trial_seed=2)
        time.sleep(3)
        if record:
    ...
    return pending_list_eb, pending_list_lm

def competitor_only(driver, url_list, comp_list, mode=1):
    # defined up-front so the final return works even when url_list is empty
    # mode 1 url_list focused search only
    # mode 2 manual_search focused search only
    # mode 3 combined search
    pending_list_lm = []
    pending_list_eb = []
    # Shared across BOTH branches: the same physical Chrome flows from the URL
    # branch into the name branch, so its memory accumulates continuously — the
    # recycle counter must too (don't reset it per branch).
    recycle_dir = os.path.dirname(os.getcwd())   # same dir main passes to build_driver
    processed = 0   # properties scraped since last driver recycle
    if url_list and (mode == 1 or mode == 3):
        ...
        try:
            ...
            start_index = 0
            if os.path.exists("checkpoint_save_competitor_url.txt"):
                with open("checkpoint_save_competitor_url.txt", "r") as f:
                    cross_check = f.read().strip()
                log.info(f"Last checkpoint URL: {cross_check}")
                
                checkpoint_idx = -1
                for idx, url in enumerate(url_list):
                    if get_base_url(url) == get_base_url(cross_check):
                        checkpoint_idx = idx
                        break
                
                if checkpoint_idx != -1:
                    if checkpoint_idx == len(url_list) - 1:
                        log.info("Last session completed the entire list successfully. Starting fresh from the beginning.")
                        if URL_REFRESHER_ACTIVE:
                            request_by_system(focus_ota="tiket")
                        start_index = 0
                    else:
                        log.info(f"Found checkpoint in list at index {checkpoint_idx}. Resuming from index {checkpoint_idx + 1}.")
                        start_index = checkpoint_idx + 1
                else:
                    log.warning("Checkpoint URL not found in the current target list. Starting fresh from the beginning.")
                    start_index = 0
            
            targets = url_list[start_index : start_index + MAX_PROPERTIES]
            print(f"[COMPETITOR SCRAPE - URL MODE] Targeting {len(targets)} hotels this session.")
            ...
            for i, url in enumerate(targets, 1):
                try:
                    # Verify driver connection health before starting property search
                    try:
                        _ = driver.current_url
                    except Exception:
                        log.warning(f"Driver connection unresponsive before target {i}. Rebuilding driver...")
                        driver = build_driver(recycle_dir, headless=IS_VPS)
                    ...
                    for vii in period_cycling:
                        ...
                        if vii <= 7:
                            try:
                                record, pending_lm = scrape_hotel_price(driver, url, vii, trial_seed=2)
                            except (WebDriverException) as driver_err:
                                log.warning(f"Driver crashed/disconnected: {driver_err}. Recovering...")
                                # Clean up and restart driver
                                driver = recycle_driver(driver, recycle_dir, IS_VPS)
                                # Retry the scrape attempt
                                record, pending_lm = scrape_hotel_price(driver, url, vii, trial_seed=2)
                            if record:
                                new_records.append(record)
                                with open("checkpoint_save_competitor_url.txt", "w") as f:
                                    f.write(f"{url}\n")
                            if pending_lm:
                                pending_list_lm.append(pending_lm)
                        elif vii > 7:
                            try:
                                record_eb, pending_eb = scrape_hotel_price_early_book(driver, url, vii, trial_seed=2)
                            except (WebDriverException) as driver_err:
                                log.warning(f"Driver crashed/disconnected: {driver_err}. Recovering...")
                                # Clean up and restart driver
                                driver = recycle_driver(driver, recycle_dir, IS_VPS)
                    ...
                    log.error(f"[COMPETITOR ONLY - URL LIST] Error on item {i} ({url}): {item_err}", exc_info=True)
                    # Check if driver died and rebuild
                    try:
                        _ = driver.current_url
                    except Exception:
                        log.warning("Driver socket disconnected during item scrape. Rebuilding driver...")
                        driver = build_driver(recycle_dir, headless=IS_VPS)
                    processed += 1
                    continue
            ...
            log.error(f"[COMPETITOR ONLY - URL LIST] Unexpected error: {e}", exc_info=True)
            return driver, [], []
        finally:
                # if driver:
                #     driver.quit()
    ...
    if comp_list and (mode == 2 or mode == 3):
        log.info("Searching manually through list.")
        def clean_property_url(url: str) -> str:
            """
            Strip query params + trailing numeric property ID.
            e.g. https://www.tiket.com/en-id/hotel/indonesia/djineng-rice-terrace-canggu-511001669192830200?room=1&...
            →  https://www.tiket.com/en-id/hotel/indonesia/djineng-rice-terrace-canggu
            """
            # 1) drop query string & fragment
            base = urlunparse(urlparse(url)._replace(query="", fragment=""))
            # 2) drop trailing slash if any, then the trailing numeric ID suffix
            base = base.rstrip("/")
            base = re.sub(r"\?.*$", "", base)
            return base
        ...
        try:
            ...
            cross_check = ""   # empty until a checkpoint file exists → comparisons stay False
            if os.path.exists("checkpoint_save_competitor_name.txt"):
                with open("checkpoint_save_competitor_name.txt", "r") as f:
                    cross_check = f.read()
                    log.info(f"cross_check is : {cross_check}")
            last_line = comp_list[-1]
            for i in range(len(comp_list)):
                cross_check_result = get_base_url(last_line) == get_base_url(cross_check)
                if comp_list[i].strip() == last_line or cross_check_result:
                    i = 0
                    break_signal = True

                if only_once == True and os.path.exists("checkpoint_save_competitor_name.txt"):
                    # result = bool(re.search(str(lines[i]), str(cross_check)))
                    # print(f"[DEBUG] result is {result}")
                    result = get_base_url(comp_list[i]) == get_base_url(cross_check)
                    if result == True:
                        log.info("Found last checkpoint!!!")
                        allowed_proceed = True
                        only_once = False
                ...
                if allowed_proceed == True or not os.path.exists("checkpoint_save_competitor_name.txt"):
                    new_urls.append(comp_list[i])
                    debug_counter += 1
            ...
            targets = new_urls[:MAX_PROPERTIES]
            print(f"[COMPETITOR MODE - NAME ONLY] Targeting {len(targets)} hotels this session.")
            ...
            for idx, url in enumerate(targets, 1):
                try:
                    ...
                    try:
                        ...
                        log.warning(f"Driver connection unresponsive before target {idx}. Rebuilding driver...")
                        driver = build_driver(recycle_dir, headless=IS_VPS)

                    # Same shared counter as the URL branch — driver memory carries over
                    if processed > 0 and processed % DRIVER_RECYCLE_EVERY == 0:
                        driver = recycle_driver(driver, recycle_dir, IS_VPS)

                    log.info(f"[{idx}/{len(targets)}] Scraping …")
                    obtained_url = manual_scrape(driver, url)
                    processed += 1        # count before the match/continue branch
                    if obtained_url != None:
                        cleaned_url = clean_property_url(obtained_url)
                        with open("checkpoint_save_competitor_name.txt", "w") as f:
                            f.write(f"{url}\n")
                        ...
                        if cleaned_url not in seen_url_ref.keys():
                            # page_urls.append(cleaned_obtained_url)
                            seen_url_ref[cleaned_url] = ""
                            with open("seen_url.json", "w") as f:
                                json.dump(seen_url_ref, f)
                            with open("URL_pool_list.txt", "a") as f:
                                f.write(f"{cleaned_url}\n")
                            log.info(f"Saved URL {cleaned_url}")
                    else:
                        continue
                except Exception as item_err:
                    log.error(f"[COMPETITOR ONLY - NAME SEARCH] Error on item {idx} ({url}): {item_err}", exc_info=True)
                    try:
                        _ = driver.current_url
                    except Exception:
                        log.warning("Driver socket disconnected during item scrape. Rebuilding driver...")
            ...
            log.error(f"[COMPETITOR ONLY - NAME SEARCH] Unexpected error: {e}", exc_info=True)
            return driver, [], []
        finally:
                # if driver:
                #     driver.quit()
                ...
                #     shutil.rmtree(tempo_prof, ignore_errors=True)
                #     log.info("Temp profile directory removed.")
                log.info("Finish processing URL named competitor.")
                log.info("=" * 50)
    return driver, pending_list_eb, pending_list_lm
...
if __name__ == "__main__":
    ...
    try:
        ...
        agoda_try = False
        print(f"Mode competitor scrape: {competitor_scrape}")
        print(f"Mode agoda scrape: {agoda_try}")
        if competitor_scrape == True:
            log.info("Updating only competitor mode.")
            captured_url, manual_find = main_processing(source="json")
            print(f"[DEBUG] captured url is : {captured_url}")
            print(f"[DEBUG] manual find is : {manual_find}")
            driver, pending_list_eb_extend, pending_list_lm_extend = competitor_only(driver, captured_url, manual_find)
        else:
            log.info("Current mode is all-in scraping")
            pending_list_eb_extend, pending_list_lm_extend = all_properties_scrape(driver)
        ...
        if agoda_try == True:
            ...
            if len(pending_list_eb_extend) > 0:
                print(f"Early book booking type to be checked by Agoda, list exist with {len(pending_list_eb_extend)} elements detected")
                dt_now = datetime.today()
                dt_now_rev = dt_now.strftime("%Y-%m-%d")
                for elem in pending_list_eb_extend:
                    if elem:
                        agoda_scrape = scrape_agoda(elem["property_name"], elem["address"], elem["period_days"], "eb")
                        results = []
                        print(f"Agoda scrape detail is : {agoda_scrape}")
                        ...
                        if type(agoda_scrape) != int:
                            for k in agoda_scrape:
                                ...
                                results.append(data)
                            to_be_continued_eb = [{"property_name": elem["property_name"], "villa_or_hotel": None,
                                                "rating": None, "total_rows": len(results), "rows": results}]
            ...
            if len(pending_list_lm_extend) > 0:
                print(f"Last Minute booking type to be checked by Agoda, list exist with {len(pending_list_lm_extend)} elements detected")
                dt_now = datetime.today()
                dt_now_rev = dt_now.strftime("%Y-%m-%d")
                for elem in pending_list_lm_extend:
                    if elem:
                        agoda_scrape = scrape_agoda(elem["property_name"], elem["address"], elem["period_days"], "lm")
                        results = []
                        print(f"Agoda scrape detail is : {agoda_scrape}")
                        ...
                        if type(agoda_scrape) != int:
                            for k in agoda_scrape:
                                ...
                                data["target_date"] = k["target_date"]
                                results.append(data)
                            to_be_continued_lm = [{"property_name": elem["property_name"], "villa_or_hotel": None,
                                                "rating": None, "total_rows": len(results), "rows": results}]
